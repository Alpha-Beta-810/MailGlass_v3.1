...
# The trained pipeline was pickled while these two classes lived in the
# training script's __main__ module, so joblib's unpickler always looks
# for `__main__.SenderPatternFeatures` / `__main__.URLFeatureExtractor`,
# no matter which module actually defines them. Registering them onto
# sys.modules['__main__'] here - unconditionally, at import time - makes
# unpickling work regardless of how this project is invoked (CLI script,
# Flask app, pytest, an interactive shell, ...), instead of only working
# when phishing_detector.py itself happens to be the entry point.
sys.modules["__main__"].SenderPatternFeatures = SenderPatternFeatures
