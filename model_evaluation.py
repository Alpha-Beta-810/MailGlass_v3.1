...
import joblib
import matplotlib
# Use a non-interactive backend so plt.savefig() never opens a GUI window.
# With an interactive backend, plt.show() blocks the terminal until every
# figure window is closed by hand before the script can continue/exit -
# the "have to close the diagrams for the terminal to move forward" issue.
# Agg renders straight to file/memory and never opens a window, so the
# script always runs start-to-finish unattended.
matplotlib.use("Agg")
...
def create_confusion_matrix(y_test, y_pred, save_plot=True):
    ...
    # No plt.show(): with the Agg backend there's no window to show, and
    # the figure is closed immediately so memory doesn't build up across
    # repeated runs/calls.
    plt.close()
